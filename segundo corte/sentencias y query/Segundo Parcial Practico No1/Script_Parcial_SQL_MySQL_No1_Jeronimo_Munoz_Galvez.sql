...
INSERT INTO pedidos VALUES
 ...
 estadoPrevio varchar(20),
 estadoNuevo varchar(20),
 foreign key (idPedidofk) references pedidos(pedido_id)
);
...
begin
-- error raro q no se que es pero estaba en el codigo pasado
    declare exit handler for sqlexception
    begin
        rollback;
        set p_mensaje = "la actualización no fue exitosa.";
    end;

-- verificar si el pedido existe + mensaje de error (a)
    declare v_cantidad int;
    declare v_estado_actual varchar(20);
    declare v_producto_id int;

    select cantidad, estado, producto_id
    into v_cantidad, v_estado_actual, v_producto_id
    from pedidos
    where pedido_id = p_id_pedido;

    if v_cantidad is null then
        set p_mensaje = "no se encuentra pedido con el id ingresado. no es posible actualizar el estado.";
    end if;
...
-- registrar cambio
    insert into log_cambios_estado(idPedidofk, estadoPrevio, estadoNuevo)
    values (p_id_pedido, v_estado_actual, p_nuevo_estado);

-- actualizar el estado del pedido (c)
    update pedidos
    set estado = p_nuevo_estado
    where pedido_id = p_id_pedido;

-- resutaurar estado si el pedido se cancella (d)
    ...
    set p_mensaje = "estado del pedido actualizado correctamente.";
