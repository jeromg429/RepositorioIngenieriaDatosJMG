...
create function fn_clasificar_medico (p_medico_id int)
returns varchar (10)
begin
	declare total_consultas_realizadas int;
    
	select count(m.medico_id) as total_consultas_realizadas;
    if total_consultas_realizadas >= 5 then return "EXPERTO";
    else if total_consultas_realizadas < 2 then return "JUNIOR";
	else return "INTEMEDIO";
    end if;
    end if;
end //
...
create view vista_rendimiento_medicos as
select 
    m.medico_id,
    m.nombre,
    m.especialidad,
    m.salario,
    count(c.medico_id),
    fn_clasificar_medico(m.medico_id),
    sum(c.costo) as total_ingresos_generados
    
from medicos m
left join consultas c on c.medico_id = m.medico_id
where fn_clasificar_medico(m.medico_id) = "EXPERTO" 
